...
public interface SpringDocMemberInfoController {
    ...
    @Parameter(name = "userid", description = "조회할 사용자 ID (본인)", in = ParameterIn.QUERY, required = true, example = "1")
    @ApiResponse(responseCode = "200", description = "유저 정보 조회에 성공했습니다.")
    @ApiErrorResponse(status = HttpStatus.BAD_REQUEST, instance = "/member/profile", errorCases = {
            @ErrorCase(description = "토큰 없음", code = 9001, exampleMessage = "토큰이 없습니다.")
    })
    @ApiErrorResponse(status = HttpStatus.UNAUTHORIZED, instance = "/member/profile", errorCases = {
            @ErrorCase(description = "유효하지 않은 토큰 형식", code = 8002, exampleMessage = "유효하지 않은 토큰 형식입니다."),
            @ErrorCase(description = "만료된 토큰", code = 8003, exampleMessage = "만료된 토큰입니다.")
    })
    @ApiErrorResponse(status = HttpStatus.FORBIDDEN, instance = "/member/profile", errorCases = {
            @ErrorCase(description = "본인만 조회 가능", code = 7001, exampleMessage = "본인의 정보만 조회할 수 있습니다.")
    })
    @ApiErrorResponse(status = HttpStatus.NOT_FOUND, instance = "/member/profile", errorCases = {
            @ErrorCase(description = "사용자 없음", code = 6001, exampleMessage = "일치하는 사용자가 존재하지 않습니다.")
    })
    @ApiErrorResponse(status = HttpStatus.INTERNAL_SERVER_ERROR, instance = "/member/profile", errorCases = {
